...
uint8_t button_getButton() {
	// Since buttons are active-low, I take the complement, since working with active-high buttons feels more natural.
	// I then bitwise and it so that I can be absolutely certain that there's no unexpected values.
	unsigned char buttons = (~GPIO_PORTE_DATA_R & 0b1111);
	// this feels inelegant but i can't come up with anything better
	if (buttons > 7) return 4; // anything 8 or higher should return 4
	if (buttons > 3) return 3; // 4, 5, 6, and 7 should return 3
	if (buttons > 1) return 2; // 2 and 3 should return 2
	return buttons; // at this point, buttons is guaranteed to be either 0 or 1, so just return that
}
